	
	
//	NIA Weapons & Magazines

	
	
	//////////////////
	//   ACR Rifles   /
	//////////////////
	
//	Classnames:							  Description:
	
	hlc_rifle_ACR68_SBR_tan      		// Remington ACR//E in 6.8mm SPCII with 10.5" Barrel (Tan)
	hlc_rifle_ACR68_SBR_black			// Remington ACR//E in 6.8mm SPCII with 10.5" Barrel (Black)
	hlc_rifle_ACR68_SBR_green			// Remington ACR//E in 6.8mm SPCII with 10.5" Barrel (Green)
	
	hlc_rifle_ACR68_Carb_black			// Remington ACR//E in 6.8mm SPCII with 14.5" Barrel (Black)
	hlc_rifle_ACR68_Carb_tan			// Remington ACR//E in 6.8mm SPCII with 14.5" Barrel (Tan)
	hlc_rifle_ACR68_Carb_green			// Remington ACR//E in 6.8mm SPCII with 14.5" Barrel (Green)
	
	hlc_rifle_ACR68_MID_black			// Remington ACR//E in 6.8mm SPCII with 16.5" Barrel (Black)
	hlc_rifle_ACR68_MID_tan				// Remington ACR//E in 6.8mm SPCII with 16.5" Barrel (Tan)
	hlc_rifle_ACR68_MID_green			// Remington ACR//E in 6.8mm SPCII with 16.5" Barrel (Green)
	
	hlc_rifle_ACR68_full_black			// Remington ACR//E in 6.8mm SPCII with 20" Barrel (Black)
	hlc_rifle_ACR68_full_tan			// Remington ACR//E in 6.8mm SPCII with 20" Barrel (Tan)
	hlc_rifle_ACR68_full_green			// Remington ACR//E in 6.8mm SPCII with 20" Barrel (Green)

	hlc_rifle_ACR_SBR_tan      			// Remington ACR//E in 5.56mm NATO with 10.5" Barrel (Tan)
	hlc_rifle_ACR_SBR_black				// Remington ACR//E in 5.56mm NATO with 10.5" Barrel (Black)
	hlc_rifle_ACR_SBR_green				// Remington ACR//E in 5.56mm NATO with 10.5" Barrel (Green)
	
	hlc_rifle_ACR_Carb_black			// Remington ACR//E in 5.56mm NATO with 14.5" Barrel (Black)
	hlc_rifle_ACR_Carb_tan				// Remington ACR//E in 5.56mm NATO with 14.5" Barrel (Tan)
	hlc_rifle_ACR_Carb_green			// Remington ACR//E in 5.56mm NATO with 14.5" Barrel (Green)
	
	hlc_rifle_ACR_MID_black				// Remington ACR//E in 5.56mm NATO with 16.5" Barrel (Black)
	hlc_rifle_ACR_MID_tan				// Remington ACR//E in 5.56mm NATO with 16.5" Barrel (Tan)
	hlc_rifle_ACR_MID_green				// Remington ACR//E in 5.56mm NATO with 16.5" Barrel (Green)
	
	hlc_rifle_ACR_full_black			// Remington ACR//E in 5.56mm NATO with 20" Barrel (Black)
	hlc_rifle_ACR_full_tan				// Remington ACR//E in 5.56mm NATO with 20" Barrel (Tan)
	hlc_rifle_ACR_full_green			// Remington ACR//E in 5.56mm NATO with 20" Barrel (Green)
	
	hlc_rifleACR_SBR_cliffhanger			// Remington ACR//E in 5.56mm NATO with 10.5" Barrel // Cliffhanger special edition	(Note: Technically Speaking, STANAG Magazines ARE capable of loading/feeding 6.8 SPC, but not PMAGs or other polymer mags.. I'm doing a mulligan to accomodate the request)
	
	hlc_30rnd_68x43_FMJ				// 26//round magazine of FMJ 6.8x43mm SPC
	hlc_30rnd_68x43_OTM				// 26//round magazine of Ballistic//Tip Boat//tail 6.8x43mm SPC
	hlc_30rnd_68x43_Tracer				// 26//round magazine of NATO//spec Tracer 6.8x43mm SPC
	hlc_30rnd_68x43_IRDIM				// 26//round magazine of IR//DIM Tracer 6.8x43mm SPC
	hlc_30rnd_68x43_MFMJ				// 26//round magazine of 1//to//4 NATO//spec Tracer/FMJ 6.8x43mm SPC
	hlc_30rnd_68x43_MIRDIM				// 26//round magazine of 1//to//4 NATO//spec IR//DIM/FMJ 6.8x43mm SPC
	
	

	//////////////////
	//   AK Rifles    /
	//////////////////
	
//	Classnames:							  Description:
	
	HLC_AK_ammobox					// Ammo box containing all this.
	
	hlc_rifle_ak74					// AK74
	hlc_rifle_ak74_MTK				// AK74 with MTK83 Picatinny Mount
	hlc_rifle_ak74_dirty				// AK74, Worn
	hlc_rifle_ak74_dirty2				// AK74, Worn
	hlc_rifle_aks74					// AKS74
	hlc_rifle_aks74_MTK				// AKS74 with MTK83 Picatinny Mount
	hlc_rifle_aks74u				// AKS74U
	hlc_rifle_aks74u_MTK				// AKS74U with MTK83 Picatinny Mount
	hlc_rifle_ak47					// AK47
	hlc_rifle_akm					// AKM
	hlc_rifle_akm_MTK				// AK47 with MTK83 Picatinny Mount
	hlc_rifle_rpk					// RPK
	hlc_rifle_ak12					// AK12
	hlc_rifle_aku12					// AK12U 
	hlc_rifle_rpk12					// RPK12
	hlc_rifle_rpk74n				// RPK74N
	hlc_rifle_ak12gl				// AK12+GP30
	hlc_rifle_akmgl					// AKM+GP25
	hlc_rifle_aks74_GL				// AKS74+GP30 (Export Variant)
	hlc_rifle_saiga12k				// Saiga12K Shotgun
	hlc_rifle_aek971				// ZID AEK971S
	hlc_rifle_aek971				// ZID AEK971S, Worn (Textuere variant)
	hlc_rifle_aek971_mtk				// ZID AEK971S, with MTK83 Picatinny Mount
	hlc_rifle_RK62					// Valmet Rk.62
	hlc_rifle_slr107u				// Arsenal Inc. SLR107U (Izhmash AKM Reciever on Bulgarian Parts)
	hlc_rifle_slr107u_MTK				// Arsenal Inc. SLR107U with MTK83 Picatinny Mount
	hlc_rifle_ak74m					// AK74M
	hlc_rifle_ak74m_gl				// AK74M+GP30
	hlc_rifle_ak74m_MTK				// AK74M with MTK83 Picatinny Mount

	HLC_Optic_PSO1					// PSO1 Sniper Optic
	HLC_Optic_1p29					// 1P29 Rifle Combat Optic
	
	hlc_muzzle_545SUP_AK				// PBS4 Suppressor (5.45x39)
	hlc_muzzle_762SUP_AK				// PBS1 Suppressor (7.62x39)
	
	hlc_optic_kobra					// Kobra Collimator Sight
	hlc_optic_goshawk				// Goshawk Thermal Sight
	
	hlc_30Rnd_545x39_B_AK	 			// 30 Round magazine of 5.45x39mm Ball Rounds (i.e, standard bullets)
	hlc_30Rnd_545x39_T_AK	 			// 30 Round magazine of 5.45x39mm Tracer Rounds
	hlc_30Rnd_545x39_EP_AK	 			// 30 Round magazine of 5.45x39mm Enhanced Performance Rounds
	
	hlc_45Rnd_545x39_t_rpk	 			// 45 Round magazine of 5.45x39mm Mixed at a ratio of 1 Tracer for every 2 Standard Ball rounds
	hlc_60Rnd_545x39_t_rpk	 			// 60 Round magazine of 5.45x39mm Mixed at a ratio of 1 Tracer for every 2 Standard Ball rounds
	
	hlc_30Rnd_762x39_b_ak	 			// 30 Round magazine of 7.62x39mm Ball Rounds (i.e, standard bullets)
	hlc_30Rnd_762x39_t_ak	 			// 30 Round magazine of 7.62x39mm Tracer Rounds
	hlc_45Rnd_762x39_t_rpk	 			// 45 Round magazine of 7.62x39mm Tracer Rounds
	
	hlc_45Rnd_762x39_m_rpk	 			// 45 Round magazine of 7.62x39mm Mixed at a ratio of 1 Tracer for every 2 Standard Ball rounds
	hlc_75rnd_762x39_m_rpk				// 75 Round magazine of 7.62x39mm Mixed at a ratio of 1 Tracer for every 2 Standard Ball rounds
	
	hlc_10rnd_12g_buck_S12				// 10 Round magazine of 12 Gauge 00//Buckshot rounds (dev note// Due to the way BIS implemented shotguns, buckshot tends to be an absolute building destroyer.)
	hlc_10rnd_12g_slug_S12				// 10 Round magazine of 12 Gauge Solid Slug rounds
	
	hlc_VOG25_AK					// HE Round for the GP25 and GP30
	
	hlc_GRD_White					// Smoke Rounds for the GP25 and GP30
	hlc_GRD_red					// Smoke Rounds for the GP25 and GP30
	hlc_GRD_green					// Smoke Rounds for the GP25 and GP30
	hlc_GRD_blue					// Smoke Rounds for the GP25 and GP30
	hlc_GRD_orange					// Smoke Rounds for the GP25 and GP30
	hlc_GRD_purple					// Smoke Rounds for the GP25 and GP30
	

	
	//////////////////
	//  AR//15 Rifles  /
	//////////////////

//	Classnames:							  Description:
	
	hlc_rifle_RU556		 			// AR15 Sanitied Carbine (Magpul AFG, MOE,P//Mag,BAD//lever. Novekse KX3)
	hlc_rifle_RU5562				// AR15 Magpul Carbine (Magpul AFG, UBR,P//Mag,BAD//lever. Novekse KX3)
	hlc_rifle_Colt727				// Colt Carbine (Colt Model 727 "Commando" // "M4" Barrel, M16A2 Upper, Full//Auto Lower)
	hlc_rifle_Colt727_GL				// Same as above, add M203
	hlc_rifle_bcmjack	 			// Bravo Company MFG/Haley Strategic 'Jack" Carbine ( Milspec M4 reciever, Geissele SMR Mk1, BCMGunfighter Muzzle brake/comp, there's more, but this isn't an advert)
	hlc_rifle_Bushmaster300				// Busmaster .300 Carbine (M4A1//profile carbine chambered for .300 Blackout, instead of 5.56mm NATO)
	hlc_rifle_vendimus				// AR15 .300 Dissipator Carbine (Carbine//length heavy barrel covered by full//length rifle furniture)
	hlc_rifle_SAMR					// Rock River Arms LAR//15 AMR (Full//Auto//Capable full//length rifle, kitted out to be able to double as a marksman rifle, with accuracy to match)
	hlc_rifle_honeybase				// AAC 'Honey//Badger',Sans Suppressor
	hlc_rifle_honeybadger				// AAC 'Honey//Badger' (Suppressed Carbine//length defense weapon, created to show off the low//velocity suppressed capabilities of the .300 Blackout round)

	hlc_muzzle_556NATO_KAC				// Sound Suppressor 5.56MM NATO AR15s
	hlc_muzzle_300blk_KAC				// Sound Suppressor .300 Blackout AR15s

	29rnd_300BLK_STANAG				// 30 Round magazine of .300 Blackout FMJ
	29rnd_300BLK_STANAG_T	 			// 30 Round magazine of .300 Blackout Tracer Rounds
	29rnd_300BLK_STANAG_S	 			// 30 Round magazine of .300 Blackout FMJ "cold" loads for optimum sound suppression.
	
	hlc_30rnd_556x45_EPR				// 30 Round magazine of 5.56x45mm NATO Enhanced Performance Rounds.
	hlc_30rnd_556x45_SOST				// 30 Round magazine of 5.56x45mm NATO Mk318 Rounds(designed for optimal performance with even SBR//length rifles).
	hlc_30rnd_556x45_SPR				// 30 Round magazine of 5.56x45mm NATO Mk262 Rounds(desgined for long range ballistic consistency).
	
	
	
	//////////////////
	//   AUG Rifles 	 /
	//////////////////
	
//	Classnames:							  Description:
	
	hlc_barrel_standard 				// Standard length(20") AUG Barrel
	hlc_barrel_carbine				// Carbine length (18") AUG Barrel 
	hlc_barrel_hbar					// Heavy AUG Barrel (24")
	hlc_barrel_9mm					// 9mm Parabellum Caliber Conversion Kit. (Contains Bolt, Magazine Adapter, 9" Barrel)
	
	hlc_rifle_aug					// Steyr AUGA1 // Rifle//barrel (Default OD)
	hlc_rifle_auga1_t				// "	"	(Tan)
	hlc_rifle_auga1_B				// "	"	(Black)
	hlc_rifle_auga1carb				// Steyr AUGA1 // Carbine//barrel
	hlc_rifle_auga1carb_t				// "	"	(Tan)
	hlc_rifle_auga1carb_b				// "	"	(Black)
	hlc_rifle_aughbar				// Steyr AUGA1 // Heavy Barrel
	hlc_rifle_aughbar_b		
	hlc_rifle_aughbar_t		
	hlc_rifle_augpara				// Steyr AUGA1 // 9mm Para
	hlc_rifle_augpara_b		
	hlc_rifle_augpara_t		
	hlc_rifle_auga2					// Steyr AUGA2 // Rifle//barrel (Default OD)
	hlc_rifle_auga2_b
	hlc_rifle_auga2_t		
	hlc_rifle_auga2carb				// Steyr AUGA2 // Carbine//barrel
	hlc_rifle_auga2carb_t
	hlc_rifle_auga2carb_b 
	hlc_rifle_auga2lsw				// Steyr AUGA2 // Heavy Barrel
	hlc_rifle_auga2lsw_t
	hlc_rifle_auga2lsw_b 
	hlc_rifle_auga2para				// Steyr AUGA2 // 9mm Para
	hlc_rifle_auga2para_b
	hlc_rifle_auga2para_t	
	hlc_rifle_augsr					// Steyr AUGSR (Special Reciever) // Rifle//barrel (Default OD)
	hlc_rifle_augsr_b
	hlc_rifle_augsr_t
	hlc_rifle_augsrcarb				// Steyr AUGSR (Special Reciever) // Carbine//barrel
	hlc_rifle_augsrcarb_t
	hlc_rifle_augsrcarb_b
	hlc_rifle_augsrhbar				// Steyr AUGSR (Special Reciever) // Heavy Barrel
	hlc_rifle_augsrhbar_b
	hlc_rifle_augsrhbar_t
	hlc_rifle_auga3					// Steyr AUGA3// Rifle//barrel (Default Green)
	hlc_rifle_auga3_bl				// "	"	"	(Blue)
	hlc_rifle_auga3_b				// "	"	"	(Black)
	hlc_rifle_auga3_GL				// Steyr AUGA3 with M203A1
	hlc_rifle_auga3_GL_BL
	hlc_rifle_auga3_GL_B
	
	hlc_30Rnd_556x45_B_AUG				// 30 Round magazine of 5.56x45mm NATO Enhanced Performance Rounds for the Steyr AUG.
	hlc_30Rnd_556x45_SOST_AUG			// 30 Round magazine of 5.56x45mm NATO Mk318 Rounds for the Steyr AUG.
	hlc_30Rnd_556x45_SPR_AUG			// 30 Round magazine of 5.56x45mm NATO Mk262 Rounds(desgined for long range ballistic consistency for the Steyr AUG.
	hlc_30Rnd_556x45_T_AUG				// 30 Round magazine of 5.56x45mm NATO Red Trace Ball for the Steyr AUG
	hlc_40Rnd_556x45_B_AUG				// 42 Round magazine of 5.56x45mm NATO Enhanced Performance Rounds for the Steyr AUG.
	hlc_40Rnd_556x45_SOST_AUG			// 42 Round magazine of 5.56x45mm NATO Mk318 Rounds for the Steyr AUG.
	hlc_40Rnd_556x45_SPR_AUG			// 42 Round magazine of 5.56x45mm NATO Mk262 Rounds for the Steyr AUG.
	hlc_25Rnd_9x19mm_M882_AUG			// 25 Round magazine of 9x19mm M882 NATO Ball for the Aug 9mm.
	hlc_25Rnd_9x19mm_JHP_AUG			// 25 Round magazine of 9x19mm LE Jacketed Hollowpoint for the Aug 9mm.
	hlc_25Rnd_9x19mm_subsonic_AUG			// 25 Round magazine of 9x19mm Subsonic Ball for the Aug 9mm.
	
	hlc_muzzle_snds_AUG				// AAC M4//2000 with thread adapter for Steyr AUG
	hlc_muzzle_snds_a6AUG				// AWC Agenda Six Suppressor threaded for the Steyr AUG 9mm.


	
	//////////////////////////
	//   AWM Sniper Rifles 	 /
	//////////////////////////
	
//	Classnames:							  Description:
	
	hlc_rifle_awcovert		 		// AI Arctic Warfare Covert//Magnum (Olive Drab) .300 Win Mag
	hlc_rifle_awcovert_BL		 		// AI Arctic Warfare Covert//Magnum (Flat Dark Earth) .300 Win Mag
	hlc_rifle_awcovert_FDE		 		// AI Arctic Warfare Covert//Magnum (Black) .300 Win Mag
	hlc_rifle_awmagnum		 		// AI Arctic Warfare Magnum (Olive Drab) .300 Win Mag // With Parker//Hale Bipod
	hlc_rifle_awmagnum_BL		 		// AI Arctic Warfare Magnum (Black) .300 Win Mag // With Parker//Hale Bipod
	hlc_rifle_awmagnum_FDE				// AI Arctic Warfare Magnum (Flat Dark Earth) .300 Win Mag // With Parker//Hale Bipod
	hlc_rifle_awMagnum_FDE_ghillie			// AI Arctic Warfare Magnum (Flat Dark Earth) .300 Win Mag // With Parker//Hale Bipod and Ghillie Shroud
	hlc_rifle_awMagnum_BL_ghillie			// AI Arctic Warfare Magnum (Black) .300 Win Mag // With Parker//Hale Bipod and Ghillie Shroud
	hlc_rifle_awMagnum_OD_ghillie			// AI Arctic Warfare Magnum (Olive Drab) .300 Win Mag // With Parker//Hale Bipod and Ghillie Shroud
	
	hlc_5rnd_300WM_FMJ_AWM				// 5//round .300WM Magazine loaded with MEN S230430 FMJ rounds.
	hlc_5rnd_300WM_AP_AWM				// 5//round .300WM Magazine loaded with MEN DM131 AP rounds.
	hlc_5rnd_300WM_BTSP_AWM	 			// 5//round .300WM Magazine loaded with 190grain Boat//Tail Soft//Tip rounds.
	hlc_5rnd_300WM_mk248_AWM			// 5//round .300WM Magazine loaded with Mk.248 Mod.1 OTM rounds.
	hlc_5rnd_300WM_SBT_AWM				// 5//round .300WM Magazine loaded with down//loaded Soft//tip rounds.(SAAMI standards and Spartan 0536 advise me not to endorse the kind of downloading that is demonstrated in this magazine).
	
	
	
	//////////////////////////
	//    FN 30//11 Rifles 	 /
	//////////////////////////

//	Classnames:							  Description:
	
	hlc_rifle_l1a1slr		 		// Enfield L1A1 SLR
	hlc_rifle_SLR		 	 		// Lithgow SLR (Australian manufactured L1A1)
	hlc_rifle_STG58F		 		// Steyr STG.58 
	hlc_rifle_FAL5061		 		// FN FAL 50.61 'Para' 
	hlc_rifle_FAL5061Rail				// FN FAL 50.61 'Para' with Picatinny Rail Interface
	hlc_rifle_FAL5000		 		// FN FAL 50.00 
	hlc_rifle_FAL5000Rail				// FN FAL 50.00 with Picatinny Rail Interface
	hlc_rifle_c1A1			 		// Canadian Arms Limited C1A1 (Canadian SLR variant, First Formally adopted FAL Rifle)
	hlc_rifle_LAR			 		// FN LAR (Light infantry rifle contracted to Israel, Differs from the Support variant by the omission of bipod and standard width barrel)
	hlc_rifle_SLRchopmod		 		// Lithgow SLR Chopmod ( Litghow SLR with the selector group from an L2A1 , Barrel sawn down and flash suppressor omitted, additional pistol grip drilled to foregrip. SASR armorer improvised solution to recon egnagements in Vietnam.)
	hlc_rifle_falosw		 		// DSA Arms FAL OSW (Offensive Suppression Weapon. Or Offensively Short Weapon)
	hlc_rifle_osw_GL		 		// DSA Arms FAL OSW + M203 GL
	
	hlc_optic_PVS4FAL				// AN/PVS4 Night Optic (Exclusive to the FALs)
	hlc_optic_suit		 	 		// SUIT Optic (Single Unit, infantry, Trilux. Exclusive to the FALs)
	
	hlc_20Rnd_762x51_B_fal	 			// 20 Round magazine of 7.62x51mm Ball Rounds (i.e, standard bullets)
	hlc_20Rnd_762x51_t_fal	 			// 20 Round magazine of 7.62x51mm Tracer Rounds
	hlc_20Rnd_762x51_S_fal	 			// 20 Round magazine of 7.62x51mm cold//loaded ball (subsonic, for use with suppressor)
	hlc_50rnd_762x51_M_FAL	 			// 50 Round magazine of 7.62x51mm Mixed at a ratio of 1 Tracer for every 5 Standard Ball rounds (X//systems drum mag, so you can have the SAW be interoperable with the rest of the squad.)

	
	
	//////////////////
	//    G3 Rifles 	 /
	//////////////////

//	Classnames:							  Description:
	
	hlc_rifle_g3sg1		 			// H&K G3SG1 (Designated marksman rifle)
	hlc_rifle_psg1		 			// H&K PSG1 (Sniper Weapon System. So dedcated to the role that the tripod that it mounts to is not integral to the weapon)
	hlc_rifle_g3a3		 			// H&K G3A3 (Infantry Rifle)
	hlc_rifle_g3a3ris	 			// H&K G3A3 + Pictatinny Rail Interface
	hlc_rifle_g3ka4					// H&K G3KA4 (Modernised,slightly shorter Infantry rifle)
	HLC_Rifle_g3ka4_GL	 			// H&K G3KA4 + M203
	hlc_rifle_hk51		 			// FR Ordnance MC51 (G3 rifle made as compact as an MP5. A Specfic request made by the SAS, apparently 50 made, but none saw combat)
	hlc_rifle_hk53					// H&K HK53 (H&K's formalised solution to the SASR's dilemma, essentially an HK33 made to fit the same size package as the MP5)
	hlc_rifle_hk53RAS	 			// H&K HK53 + RAS
	hlc_rifle_hk33a2	 			// H&K HK33A2 (H&K's formal adaptation of the G3 Rifle to the 5.56mmx45 NATO standard. A2 variant with Burst triggergroup)
	hlc_rifle_hk33a2RIS	 			// H&K HK33A2 + Picatinny Rail Interface
	hlc_rifle_psg1A1	 			// H&K PSG1A1 ("Modernized" PSG1. Features Side//folding SG550 Sniper stock, and regular G3 Clawmount facilities)
	hlc_rifle_PSG1A1_RIS				// H&K PSG1A1 + Picatinny Rail Interface
	hlc_rifle_g3a3v		 			// H&K G3A3 (Infantry Rifle)
	
	HLC_Optic_ZFSG1					// Zeiss Diavari 1.5//6x Rifle Optic in high//profile mount (Modern day as//equivelant to the ZFSG1. Altscope mode uses the Mount's peephole)
	hlc_optic_accupoint_g3				// Trijicon Accupoint TR20 Rifle optic in high//profile mount (3//9x, Illuminated post reticle.Altscope mode uses the Mount's peephole)
	
	hlc_20Rnd_762x51_B_G3	 			// 20 Round magazine of 7.62x51mm Ball Rounds (i.e, standard bullets)
	hlc_20rnd_762x51_T_G3	 			// 20 Round magazine of 7.62x51mm Tracer Rounds
	hlc_50rnd_762x51_M_G3	 			// 50 Round magazine of 7.62x51mm Mixed at a ratio of 1 Tracer for every 5 Standard Ball rounds (X//systems drum mag, so you can have the SAW be interoperable with the rest of the squad.)
	
	
	
	//////////////////
	//   G36 Rifles 	 /
	//////////////////
	
//	Classnames:							  Description:
	
	hlc_rifle_G36A1					// G36 Rifle in it's intial Bundeswehr spec. S//E//F Triggerpack.
	hlc_rifle_G36A1AG36				// G36 Rifle with AG36 Launcher
	hlc_rifle_G36KA1				// G36K Carbine.S//E//F Triggerpack.
	hlc_rifle_G36C					// G36C Compact Carbine. 1//2//A Pictogram Triggerpack, Ironsights and optic mounting rail.
	hlc_rifle_G36E1					// G36 Rifle as sold by H&K for Export. 1//2//A Pictogram Triggerpack.
	hlc_rifle_G36E1AG36				// G36E Rifle with AG36 Launcher
	hlc_rifle_G36KE1				// G36KE Carbine.1//2//A Pictogram Triggerpack.
	hlc_rifle_G36V					// G36 Modernised Export Model. 1//2//A Pictogram Triggerpack, Ironsights and optic mounting rail.
	hlc_rifle_G36KV					// G36K Modernised Export Model. 1//2//A Pictogram Triggerpack, Ironsights and optic mounting rail.
	hlc_rifle_G36CV					// G36C Modernised Export Model. 1//2//A Pictogram Triggerpack, Ironsights and optic mounting rail. 
	hlc_rifle_G36VAG36				// G36V with AG36 Launcher.
	hlc_rifle_G36TAC				// G36V Rifle with Knight's Armament interface gear.
	hlc_rifle_G36KTAC				// G36K	"	"	"	"	"	"
	hlc_rifle_G36CTAC				// G36C	"	"	"	"	"	"
	hlc_rifle_G36MLIC				// G36 Rifle fitted with the STANAG 2020 Caseless Mid//Life Improvement Kit.
	hlc_rifle_G36KMLIC				// G36K	"	"	"	"	"	"	"	"	"	
	hlc_rifle_G36CMLIC				// G36C  "	"	"	"	"	"	"	"	"
	hlc_rifle_G36MLIAG36				// G36//MLI(C) with AG36	
	hlc_rifle_MG36					// MG36A1 Light Support Weapon
	
	HLC_Optic_G36dualoptic35x			// Hensoldt//Wetzlar Modular Sighting System for the G36 Rifles (Standard 3.5x)
	HLC_Optic_G36dualoptic35x2d			// Hensoldt//Wetzlar Modular Sighting System for the G36 Rifles (Standard 3.5x,2D Reticle)
	HLC_Optic_G36Export35x				// Hensoldt//Wetzlar Export Sighting System for the G36 Rifles (Standard 3.5x)
	HLC_Optic_G36Export35x2d			// Hensoldt//Wetzlar Export Sighting System for the G36 Rifles (Standard 3.5x,2D Reticle)
	HLC_Optic_G36Dualoptic15x			// Hensoldt//Wetzlar Modular Sighting System for the G36 Rifles (Replacement 1.5x)
	HLC_Optic_G36Dualoptic15x2d			// Hensoldt//Wetzlar Modular Sighting System for the G36 Rifles (Replacement 1.5x,2D Reticle)	
	HLC_Optic_G36Export15x				// Hensoldt//Wetzlar Export Sighting System for the G36 Rifles (Replacement 1.5x)
	HLC_Optic_G36Export15x2d			// Hensoldt//Wetzlar Export Sighting System for the G36 Rifles (Standard 1.5x,2D Reticle)
	
	hlc_30rnd_556x45_EPR_G36			//	
	hlc_30rnd_556x45_SOST_G36			//
	hlc_30rnd_556x45_SPR_G36			//
	hlc_100rnd_556x45_EPR_G36			// C//MAG
	
	
	
	//////////////////
	//   M14 Rifles 	 /
	//////////////////
	
//	Classnames:							  Description:
	
	hlc_rifle_M14		 			// M14 (Infantry Rifle)
	hlc_rifle_M21		 			// M21 Marksman Rifle ( Removal of select fire, Addition of Fibreglass stock and Harris Bipod)
	hlc_rifle_M14DMR		 		// M14 DMR (USMC Designated Marksman Weapon. ArmA2 DMR)
	hlc_rifle_m14sopmod	 			// Troy M14 SOPMOD (Precursor to the EBR. Modernised M14 Chasis)
	
	hlc_muzzle_snds_M14				// Sound Suppressor for the M14 Rifles 
	
	hlc_optic_artel_m14				// Redfield AR//TEL Optic (3//9x Scope. Sadly, it's impossible in the engine to replicate the locked zoom:zero function)
	hlc_optic_LRT_m14				// Leupold LR/T (Effectively the same as the LRPS that's already ingame)
	
	hlc_20Rnd_762x51_B_M14	 			// 20 Round magazine of 7.62x51mm Ball Rounds (i.e, standard bullets)
	hlc_20rnd_762x51_T_M14	 			// 20 Round magazine of 7.62x51mm Tracer Rounds
	hlc_50rnd_762x51_M_M14	 			// 50 Round magazine of 7.62x51mm Mixed at a ratio of 1 Tracer for every 5 Standard Ball rounds (X//systems drum mag, so you can have the SAW be interoperable with the rest of the squad.)

	
	
	/////////////////////////////
	//  M60 Light Machine Gun   /
	/////////////////////////////
	
//	Classnames:							  Description:
	
	hlc_lmg_M60E4		 			// M60E4
	hlc_lmg_m60					// M60 (original variant, no rails, longer, heavier)
	
	hlc_100Rnd_762x51_B_M60E4	 		// 100 Round magazine of 7.62x51mm Ball Rounds (i.e, standard bullets)
	hlc_100Rnd_762x51_T_M60E4	 		// 100 Round magazine of 7.62x51mm Tracer Rounds
	hlc_100Rnd_762x51_M_M60E4	 		// 100 Round magazine of 7.62x51mm Mixed at a ratio of 1 Tracer for every 2 Standard Ball rounds

	
	
	//////////////////////
	//   MG42s/MG42 MMGs  /
	//////////////////////
	
//	Classnames:							  Description:
	
	hlc_lmg_MG42				 	// MG42 (Wooden Stock)
	hlc_lmg_mg42_bakelite			 	// MG42 (Bakelite)
	hlc_lmg_MG3				 	// Rheinmetall MG3 
	hlc_lmg_MG3_optic				// Rheinmetall Mg M/62 (Danish Contract MG3+Optic Mount)
	hlc_lmg_MG3KWS				 	// Rheinmetall MG3//KWS : Kampfwertsteigerung (Combat Improvements Model)(Tan/RAL)
	hlc_lmg_MG3KWS_b			 	// Rheinmetall MG3//KWS : Kampfwertsteigerung (Combat Improvements Model)(Green/RAL)
	hlc_lmg_MG3KWS_g			 	// Rheinmetall MG3//KWS : Kampfwertsteigerung (Combat Improvements Model)(Black)
	hlc_lmg_MG42KWS_t			 	// Rheinmetall MG42//KWS : MG3KWS fitted with MG42 components (Tan/RAL)
	hlc_lmg_MG42KWS_g				// Rheinmetall MG42//KWS : MG3KWS fitted with MG42 components (Green/RAL)
	hlc_lmg_MG42KWS_B			 	// Rheinmetall MG42//KWS : MG3KWS fitted with MG42 components (Black)
	
	hlc_50Rnd_792x57_B_MG42			 	// Gurtrommel with 50 Rounds of 7.92x57mm sS Patrone (FMJ)				 // MG42
	hlc_50Rnd_792x57_M_MG42			 	// Gurtrommel with 50 Rounds of 7.92x57mm S.m.K //v and S.m.K Leuchtspur (EPR/Tracer 4:1)	 // MG42
	hlc_50Rnd_792x57_T_MG42			 	// Gurtrommel with 50 Rounds of 7.92x57mm S.m.K Leuchtspur (Tracer)				 // MG42
	hlc_100Rnd_792x57_B_MG42		 	// 100 Rounds of 7.92x57mm sS Patrone (FMJ)						 // MG42
	hlc_100Rnd_792x57_M_MG42		 	// 100 Rounds of  7.92x57mm S.m.K //v and S.m.K Leuchtspur (EPR/Tracer 4:1)	 	 	 // MG42	
	hlc_100Rnd_792x57_T_MG42			// 100 Rounds of  7.92x57mm S.m.K Leuchtspur (Tracer)					 // MG42
	hlc_200Rnd_792x57_B_MG42		 	// 200 Rounds of 7.92x57mm sS Patrone (FMJ)						 // MG42
	hlc_200Rnd_792x57_M_MG42		 	// 200 Rounds of  7.92x57mm S.m.K //v and S.m.K Leuchtspur (EPR/Tracer 4:1)			 // MG42
	hlc_200Rnd_792x57_T_MG42			// 200 Rounds of  7.92x57mm S.m.K Leuchtspur (Tracer)					 // MG42
	hlc_50Rnd_792x57_AP_MG42		 	// Gurtrommel with 50 Rounds of 7.92x57mm S.m.K.H (AP)					 // MG42
	hlc_100Rnd_792x57_AP_MG42		 	// 100 Rounds of 7.92x57mm S.m.K.H (AP)						 // MG42
	hlc_200Rnd_792x57_AP_MG42		 	// 200 Rounds of 7.92x57mm S.m.K.H (AP)	 					 // MG42
	hlc_50Rnd_762x51_B_MG3			 	// Gurtrommel with 50 Rounds of 7.62x51mm M855A1 in DM1 Links (FMJ)
	hlc_50Rnd_762x51_M_MG3			 	// Gurtrommel with 50 Rounds of 7.62x51mm M855A1 in DM1 Links (FMJ/Tracer 4:1)
	hlc_50Rnd_762x51_Barrier_MG3			// Gurtrommel with 50 Rounds of 7.62x51mm Mk.319 EPBR in DM1 Links (Mk319/Tracer 4:1)
	hlc_50Rnd_762x51_T_MG3			 	// Gurtrommel with 50 Rounds of 7.62x51mm M64 in DM1 Links (Tracer)	
	hlc_100Rnd_762x51_B_MG3			 	// 100 Rounds of 7.62x51mm M855A1 in DM1 Links (FMJ)
	hlc_100Rnd_762x51_M_MG3			 	// 100 Rounds of 7.62x51mm M855A1 in DM1 Links (FMJ/Tracer 4:1)
	hlc_100Rnd_762x51_Barrier_MG3			// 100 Rounds of 7.62x51mm Mk.319 EPBR in DM1 Links (Mk319/Tracer 4:1)
	hlc_100Rnd_762x51_T_MG3			 	// 100 Rounds of 7.62x51mm M64 in DM1 Links (Tracer)	
	hlc_250Rnd_762x51_B_MG3			 	// 200 Rounds of 7.62x51mm M855A1 in DM1 Links (FMJ)
	hlc_250Rnd_762x51_M_MG3			 	// 200 Rounds of 7.62x51mm M855A1 in DM1 Links (FMJ/Tracer 4:1)
	hlc_250Rnd_762x51_Barrier_MG3			// 200 Rounds of 7.62x51mm Mk.319 EPBR in DM1 Links (Mk319/Tracer 4:1)
	hlc_1250Rnd_762x51_T_MG3			// 200 Rounds of 7.62x51mm M64 in DM1 Links (Tracer)
	
	
	
	
	//////////////
	//   Minigun  /
	//////////////
	
//	Classnames:							  Description:
	
	HLC_wp_M134Painless		 		// M134D on Man//portable Custom//built Carrying rig. Aka M134D "Epilator"
	hlc_500rnd_762x51_Belt				// 500 Round magazine of 7.62x51mm rounds.
	hlc_500rnd_762x51_Cinematic	 		// 500 Round magazine of 7.62x51mm rounds, with explosive value for cinematic dramatic effect. (Wouldn't recommend for normal use, KILLS THE FUCK OUT OF FRAME RATE)

	
	
	//////////////////
	//    MP5 SGMs	 /
	//////////////////

//	Classnames:							  Description:
	
	hlc_muzzle_Agendasix				// Suppressor for all 9x19mm Mp5s EXCEPT MP5k/MP5k//pdw (HK three//lug mount)
	hlc_muzzle_Tundra				// Suppressor for all 9x19mm Mp5s
	hlc_muzzle_Agendasix10mm			// Suppressor for MP5/10
	
	hlc_acc_Surefiregrip				// Surefire Weaponlight for M55A2 and A3
	
	hlc_smg_mp5k_PDW				// MP5K//PDW (Primary Weapon Slot)
	hlc_smg_mp5k					// MP5K (Secondary weapon slot. Warning: Visual overstretch on left arm, nothing I can do about it without an overhaul to the way Arma handles pistol animations. This is why it's not in the ammo box)
	hlc_smg_mp5a2					// MP5A2 (Full//Stock, SEF Triggerpack)
	hlc_smg_mp5a3					// MP5A3 (Retractable Stock, SEF Triggerpack)

//	====The above do NOT accept side//rail attachments. The below ACCEPT side//rail attachments====

	hlc_smg_mp5a4					// MP5A4 (Full Stock, "Navy" Triggerpack)
	hlc_smg_mp5n					// MP5N (Telescoping stock, "Navy" Triggerpack)
	hlc_smg_mp510					// MP5/10 (Full Stock, "Navy" Triggerpack,10mm Auto)
	hlc_smg_mp5sd5					// MP5SD5 (Full Stock, "Navy" Triggerpack, integrally suppressed)
	hlc_smg_mp5sd6					// MP5SD5 (Telescoping stock, "Navy" Triggerpack, integrally suppressed)
	hlc_smg_9mmar					// 'Navy' SMG (Homage to Half//life. MP5N with permenantly affixed KAC tri//rail and M203//PI. Now someone jsut needs to make the Grunts, a black//mesa//esque New Mexico map and we're golden.)
	hlc_30Rnd_9x19_B_MP5	 			// 30 Round MP5 Magazine of 9x19mm M882 NATO Ball
	hlc_30Rnd_9x19_GD_MP5	 			// 30 Round MP5 Magazine of 9x19mm LE Jacketed Hollowpoints
	hlc_30Rnd_9x19_SD_MP5	 			// 30 Round MP5 Magazine of 9x19mm M882 Ball loaded to subsonic rates
	hlc_30Rnd_10mm_B_MP5	 			// 30 Round MP5 Magazine of 10mm Auto FMJ
	hlc_30Rnd_10mm_JHP_MP5				// 30 Round MP5 Magazine of 10mm Auto LE Jacketed Hollowpoints
	
	
	
	///////////////////
	//      SAWs	  /
	///////////////////
	
//	Classnames:							  Description:
	
	hlc_lmg_minimipara		 		// Minimi Para (13.7" Barrel/ Para Stock) (No Optic Mount)
	hlc_lmg_minimi					// Minimi (18.3" Barrel/Para Stock) (No Optic Mount)
	hlc_lmg_minimi_railed				// Minimi RIS (18.3" Barrel/Solid Stock/M1913 Optics Mount)
	hlc_lmg_m249para			 	// M249 Para (13.7" Barrel/Para Stock) (No Optic Mount)
	hlc_lmg_M249E2			 		// M249E2 (18.3" Barrel/Solid Stock) (No Optic Mount)
	hlc_m249_pip1			 		// M249E2 (18.3" Barrel/Solid Stock/M1913 Optics Mount)
	hlc_m249_pip2			 		// M249 Para (13.7" Barrel/Para Stock/ M1913 Optics Mount)
	hlc_m249_pip3			 		// M249 Para PIP (13.7" Barrel/PIP Stock/M1913 Optics Mount)
	hlc_m249_pip4			 		// M249 PIP (18.3" Barrel/PIP Stock/ M1913 Optics Mount/ Forward M1913 Tri//rail+VFG) (No Bipod)
	hlc_200rnd_556x45_M_SAW	 			// 200 Round magazine of 5.56x45mm M855A1 EP and M856 Tracer Rounds Mixed at a ratio of 1 Tracer for every 4 Ball rounds
	hlc_200rnd_556x45_T_SAW	 			// 200 Round magazine of 5.56x45mm M856 Tracer Rounds
	hlc_200rnd_556x45_B_SAW	 			// 200 Round magazine of 5.56x45mm M855A1 EP Rounds
	
	
	
	///////////////////////////
	//	SG510 Rifles	  /
	///////////////////////////
	
//	Classnames:							  Description:
	
	hlc_rifle_STGW57				// SIG Sturmgewehr 57/Fass 57.Can mount Kern AARAU 4x24 Scope. 7.5x55mm
	hlc_rifle_stgw57_RIS				// SIG Sturmgewehr 57/Fass 57 with Richner//Waffen Picatinny Rail Mount (if you're Swiss and you're a sport shooter, please give them your business :) )
	hlc_rifle_stgw57_commando			// Customised STGW 57, with Cut//down barrel. Inspired by Swiss gunsmith Rudolf Aebi.
	hlc_rifle_sig5104	 			// SIG 510/4. 7.62x51mm NATO
	hlc_rifle_amt					// Sig AMT (American Match Target) in 7.62 NATO. No Bipod. 
	hlc_optic_Kern					// The Kern AARAU Scope on "Butter//dish" Scope mount. 
	hlc_optic_Kern2d				//  "	"	"	"	"	"	" . 2D Reticle
	hlc_24Rnd_75x55_B_stgw	 			// 24 Round Stgw57 magazine of GP11 175gr Military FMJ 
	hlc_24Rnd_75x55_ap_stgw	 			// 24 Round Stgw57 magazine of GP11 180gr Armour//piercing Boat//Tail
	hlc_20Rnd_762x51_b_amt				// 20 Round SG510//4 magazine of 7.62x51mm Enhanced FMJ
	hlc_20Rnd_762x51_mk316_amt			// 20 Round SG510//4 magazine of 7.62x51mm mk.316 Match
	hlc_20Rnd_762x51_bball_amt			// 20 Round SG510//4 magazine of 7.62x51mm mk.319 SOST Barrier Ball
	hlc_20Rnd_762x51_T_amt				// 20 Round SG510//4 magazine of 7.62x51mm Tracer rounds
	hlc_24Rnd_75x55_T_stgw				// 24 Round  Stgw57 magazine of GP11 Tracer rounds
	
	
	
	/////////////////////////////////
	//   M1903 Springfield Rifles	/
	/////////////////////////////////
	
//	Classnames:							  Description:
	
	hlc_rifle_M1903A1_unertl			// M1903A1 Rifle permenantly fitted with 8x Unertl Scope
	hlc_rifle_M1903A1				// M1903A1 Service Rifle
	hlc_5rnd_3006_1903				// 5//round clip of .30//06 M1 Military Ball.



	//////////////////////////
	//      P226 Pistols	 /
	//////////////////////////

//	Classnames:							  Description:
	
	hlc_pistol_P226WestGerman			// SIG//Sauer P226 (Orig. West//German Run). 9x19mmNATO
	hlc_pistol_P226US				// SIG//Sauer P226 (Initial Exeter Production). 9x19mmNATO
	hlc_pistol_P226R				// SIG//Sauer P226R (Mid//2000s//Current Production). 9x19mmNATO
	hlc_pistol_P226R_Elite				// SIG//Sauer P226R Elite (Mid//2000s//Current Production, Lengthened Beavertail). 9x19mmNATO
	hlc_pistol_P226R_Combat				// SIG//Sauer P226R Combat (Mid//2000s//Current Production, Threaded Barrel). 9x19mmNATO
	hlc_pistol_Mk25				       	// Mk.25 Mod.0 (Mid//2000s//Current Production, as adopted by US Navy). 9x19mmNATO
	hlc_pistol_Mk25D				// Sig//Sauer USA Mk25D (Commercial reproduction, Sand/Tan coat). 9x19mmNATO
	hlc_Pistol_mk25RX				// Sig//Sauer USA Mk25D (Commercial reproduction, Sand/Tan coat, Romeo//1 RDS and Threaded Barrel). 9x19mmNATO
	hlc_pistol_P226R_40				// SIG//Sauer P226R (Mid//2000s//Current Production). .40 S&W
	hlc_pistol_P226R_40Elite			// SIG//Sauer P226R Elite (Mid//2000s//Current Production, Lengthened Beavertail). .40 S&W
	hlc_pistol_P226R_40Combat			// SIG//Sauer P226R Combat (Mid//2000s//Current Production, Threaded Barrel). .40 S&W
	hlc_pistol_P226R_40Enox				// SIGArms P226R Enox (Late//2000s Limited//Sale Custom). .40 S&W

	hlc_Pistol_P228					// SIG//Sauer P228 (Orig. West//German Run). 9x19mmNATO
	hlc_Pistol_M11					// SIG//Sauer M11 (Orig. West//German Run,US DOD Markings). 9x19mmNATO
	hlc_pistol_P229R				// SIG//Sauer P229R (Mid//2000s//Current Production). 9x19mmNATO
	hlc_pistol_P229R_Combat				// SIG//Sauer P229R Combat (Mid//2000s//Current Production,Threaded Barrel). 9x19mmNATO
	hlc_Pistol_M11A1				// Sig//Sauer USA M11A1 (Commercial reproduction). 9x19mmNATO
	hlc_Pistol_M11A1D				// Sig//Sauer USA M11A1D (Commercial reproduction,Sand/Tan finish). 9x19mmNATO
	hlc_pistol_P229R_Elite				// SIG//Sauer P229R Elite (Late//2000s//Current Production, Lengthened Beavertail). 9x19mmNATO
	hlc_pistol_P229R_40				// SIG//Sauer P229R (Mid//2000s//Current Production). .40 S&W
	hlc_pistol_P229R_40Combat			// SIG//Sauer P229R Combat (Mid//2000s//Current Production,Threaded Barrel). .40 S&W
	hlc_pistol_P229R_40Elite			// SIG//Sauer P229R Elite (Mid//2000s//Current Production, Lengthened Beavertail). .40 S&W
	hlc_pistol_P229R_40Enox				// SIGArms P229R Enox (Late//2000s Limited//Sale Custom). .40 S&W
	hlc_pistol_P229R_357				// SIG//Sauer P229R (Mid//2000s//Current Production). .357 SIG
	hlc_pistol_P229R_357Combat			// SIG//Sauer P229R Combat (Mid//2000s//Current Production,Threaded Barrel). .357 SIG
	hlc_pistol_P229R_357Elite			// SIG//Sauer P229R Elite (Mid//2000s//Current Production, Lengthened Beavertail). ..357 SIG
	hlc_pistol_P229R_357Stainless			// SIG//Sauer P229R Stainless (Stainless//Steel Slide, Nickel Finish). .357 SIG
	hlc_pistol_P226R_357				// SIG//Sauer P226R (Mid//2000s//Current Production). .357 SIG
	hlc_pistol_P226R_357Combat			// SIG//Sauer P226R Combat (Mid//2000s//Current Production,Threaded Barrel). .357 SIG
	hlc_pistol_P226R_357Elite			// SIG//Sauer P226R Elite (Mid//2000s//Current Production, Lengthened Beavertail). ..357 SIG
	hlc_pistol_P226R_357Stainless			// SIG//Sauer P226R Stainless (Stainless//Steel Slide, Nickel Finish). .357 SIG

//	!BONUS!

	hlc_pistol_P239					// SIG//Sauer P239 (Sub//compact). 9x19mmNATO
	hlc_pistol_P239_40				// SIG//Sauer P239 (Sub//compact). .40 S&W
	hlc_pistol_P239_357				// SIG//Sauer P239 (Sub//compact). .357 SIG


	hlc_15Rnd_9x19_B_P226				// 15 Round P226 magazine of 9x19mm M882 Ball
	hlc_15Rnd_9x19_JHP_P226				// 15 Round P226 magazine of 9x19mm Mk243 Mod 0 JHP
	hlc_15Rnd_9x19_SD_P226				// 15 Round P226 magazine of 9x19mm Subsonic FMJ
	hlc_12Rnd_40SW_B_P226				// 12 Round P226 magazine of .40 S&W Mk309 Mod 0 Ball
	hlc_12Rnd_40SW_JHP_P226				// 12 Round P226 magazine of .40 S&W Mk308 Mod 0 JHP
	hlc_12Rnd_40SW_SD_P226				// 12 Round P226 magazine of .40 S&W JHP Subsonic
	hlc_12Rnd_357SIG_B_P226				// 12 Round P226 magazine of .357SIG FMJ//FN
	hlc_12Rnd_357SIG_JHP_P226			// 12 Round P226 magazine of .357SIG JHP

	hlc_13Rnd_9x19_B_P228				// 13 Round P228 magazine of 9x19mm M882 Ball
	hlc_13Rnd_9x19_JHP_P228				// 13 Round P228 magazine of 9x19mm Mk243 Mod 0 JHP
	hlc_13Rnd_9x19_SD_P228				// 13 Round P228 magazine of 9x19mm Subsonic FMJ
	hlc_10Rnd_40SW_B_P229				// 10 Round P229 magazine of .40 S&W Mk309 Mod 0 Ball
	hlc_10Rnd_40SW_JHP_P229				// 10 Round P229 magazine of .40 S&W Mk308 Mod 0 JHP
	hlc_10Rnd_40SW_SD_P229				// 10 Round P229 magazine of .40 S&W JHP Subsonic
	hlc_10Rnd_357SIG_B_P229				// 10 Round P229 magazine of .357SIG FMJ//FN
	hlc_10Rnd_357SIG_JHP_P229			// 10 Round P229 magazine of .357SIG JHP

	hlc_10Rnd_9x19_B_P239				// 10 Round P239 magazine of 9x19mm M882 Ball
	hlc_10Rnd_9x19_JHP_P239				// 10 Round P239 magazine of 9x19mm Mk243 Mod 0 JHP
	hlc_10Rnd_9x19_SD_P239				// 10 Round P239 magazine of 9x19mm Subsonic FMJ
	hlc_8Rnd_40SW_B_P239				// 8 Round P239 magazine of .40 S&W Mk309 Mod 0 Ball
	hlc_10Rnd_40sw_JHP_P239				// 8 Round P239 magazine of .40 S&W Mk308 Mod 0 JHP
	hlc_10Rnd_40sw_SD_P239				// 8 Round P239 magazine of .40 S&W JHP Subsonic
	hlc_8Rnd_357SIG_B_P239				// 8 Round P239 magazine of .357SIG FMJ//FN
	hlc_8Rnd_357SIG_JHP_P239			// 8 Round P239 magazine of .357SIG JHP
